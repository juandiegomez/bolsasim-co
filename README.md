...
El **Slice 0 — Foundation** proporciona una aplicación Next.js ejecutable, configuración validada, health check, conexión PostgreSQL/Drizzle, migraciones y pruebas. La documentación bajo [`docs/`](docs/) conserva la baseline de producto. Portafolio, compras e histórico todavía no están implementados; no hay proveedor de mercado aprobado.
...
`DATABASE_URL` es obligatoria. Una URL inválida, otra zona horaria o un LOG_LEVEL no admitido impide el arranque con `CONFIGURATION_INVALID` y nombres de variables, sin imprimir valores. APP_TIME_ZONE tiene default America/Bogota y LOG_LEVEL default info. Las variables de mercado, usuario y depósito están reservadas y no activan funcionalidades.
...
`npm run db:migrate` aplica el journal de Drizzle y una migración inicial sin tablas de negocio; repetirla no agrega registros. No hay migración automática al iniciar HTTP. `npm run db:generate` prepara las migraciones de esquemas futuros; con el esquema vacío puede no generar cambios. No usar `push` como reemplazo de migraciones versionadas.
...
Consulta [ADR-0009](docs/adr/0009-slice-zero-foundation.md) para package manager, liveness y migración sin esquema de negocio, y [validación del Slice 0](docs/testing/slice-zero-validation.md) para evidencia y límites. ADR-0005 permanece abierto; no bloquea Foundation.
