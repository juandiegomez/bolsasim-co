...
La alerta moderada GHSA-67mh-4wv8-2f99 en la cadena de desarrollo `drizzle-kit` → `@esbuild-kit` → `esbuild` se resolvió manteniendo `drizzle-kit@0.31.10` (compatible con `drizzle-orm@0.45.2`) y aplicando en `package.json` un override de `esbuild` a `0.25.12` para la dependencia transitiva antigua. `npm audit` y `npm audit --omit=dev` quedan en cero; `db:generate` y los gates de calidad fueron verificados. No se utiliza el servidor de esbuild ni Drizzle Studio.
